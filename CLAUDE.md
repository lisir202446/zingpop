...
| 短信 | 华为云短信（代码已写，凭证未配置） |
| 支付 | 微信支付 H5 + 支付宝（代码已有，但生产支付未接入：商户号、生产凭证、回调验签和真实支付闭环未验证） |
...
- [x] 微信支付 H5 基础代码（`packages/console/core/src/pay/wechat.ts`，生产商户号、凭证、回调和真实支付闭环未完成）
- [x] 支付宝支付基础代码（`packages/console/core/src/pay/alipay.ts`，生产商户号、凭证、回调和真实支付闭环未完成）
...
6. 支付没有完成生产接入：
   - 微信支付商户申请、生产 `WECHAT_PAY_*` 凭证和 `WECHAT_PAY_NOTIFY_URL` 未验证。
   - 支付宝商户申请、生产 `ALIPAY_*` 凭证、`ALIPAY_NOTIFY_URL` 和 `ALIPAY_RETURN_URL` 未验证。
   - 真实支付下单、支付页面跳转、回调验签、订单状态更新、余额入账、重复回调幂等、金额篡改防护未完成生产闭环。
   - Lite/订阅支付还没迁移到国内支付，当前代码仍会抛出 “Lite checkout has not been migrated to domestic payments yet”。
   - 退款、发票/税务、对账、支付争议和客服处理流程未完成。
   - 旧的 Stripe 订阅/自动充值逻辑仍在代码中，面向中国市场收费前需要确认是替换、禁用还是保留为海外路径。
7. 首页页脚是否展示 ICP 备案号。
8. MySQL、`/srv/zingpop`、`/etc/zingpop/zingpop.env` 的备份策略。
9. 面向公开多用户前，必须完成用户 workspace / session / file / terminal 隔离。
...
2026-05-15 追加：不要把支付基础代码当成已商业化接入。当前只说明 WeChat Pay / Alipay 的代码路径和回调路由存在，尚未证明生产商户、真实支付、订单、余额、订阅、退款、发票和对账可用。商业化上线前以 `docs/commercialization-readiness.md` 和 `docs/production-readiness.md` 的支付清单为准。
