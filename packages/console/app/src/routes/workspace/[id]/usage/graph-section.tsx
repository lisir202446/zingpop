  ...
  return withActor(async () => {
    ...
    const keysData = await Database.use((tx) =>
      tx
        ...
        .innerJoin(AuthTable, and(eq(UserTable.accountID, AuthTable.accountID), eq(AuthTable.provider, "phone")))
        .where(
          and(
    ...
    return {
      ...
      keys: keysData.map((key) => ({
        id: key.keyId,
        displayName: key.userLogin ? `${key.userLogin} - ${key.keyName}` : key.keyName,
