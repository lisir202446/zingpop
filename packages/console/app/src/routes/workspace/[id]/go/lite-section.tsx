...
import { IconAlipay, IconWechat } from "~/component/icon"
...
const createLiteCheckoutUrl = action(
  async (workspaceID: string, successUrl: string, cancelUrl: string, method?: "alipay" | "wechat") => {
    "use server"
    const request = getRequestEvent()?.request
    const userAgent = request?.headers.get("user-agent") ?? ""
    const forwarded = request?.headers.get("x-forwarded-for")?.split(",")[0]?.trim()
    const clientIP = forwarded ?? request?.headers.get("x-real-ip") ?? "127.0.0.1"
      ...
      await withActor(
        () =>
          Billing.generateLiteCheckoutUrl({
            ...
            method,
            clientIP,
            mobile: /Android|iPhone|Mobile/i.test(userAgent),
          })
...
export function LiteSection() {
  ...
  const [store, setStore] = createStore({
    loading: undefined as undefined | "session" | "checkout" | "alipay" | "wechat",
    showModal: false,
  })
  ...
  async function onClickSubscribe(method?: "alipay" | "wechat") {
            ...
            >
              <span>{i18n.t("workspace.lite.promo.otherMethods")}</span>
                <span data-slot="other-methods-icons">
                  ...
                  <span> </span>
                  <IconWechat style={{ width: "16px", height: "16px" }} />
                </span>
              </button>
          </div>
          <Modal
          ...
          >
            <div data-slot="modal-actions">
              ...
              <button
                ...
                data-color="ghost"
                disabled={checkoutSubmission.pending || busy()}
                onClick={() => onClickSubscribe("wechat")}
              >
                <Show when={store.loading !== "wechat"}>
                  <IconWechat style={{ width: "16px", height: "16px" }} />
                </Show>
                {store.loading === "wechat" ? i18n.t("workspace.lite.promo.subscribing") : "WeChat Pay"}
              </button>
            </div>
          </Modal>
        </section>
