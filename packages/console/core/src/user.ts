...
export namespace User {
    ...
    Database.use((tx) =>
      tx
        .select({
          ...
          authLogin: AuthTable.subject,
        })
        .from(UserTable)
        .leftJoin(AuthTable, and(eq(UserTable.accountID, AuthTable.accountID), eq(AuthTable.provider, "phone")))
  ...
  export const getAuthPhone = fn(z.string(), (id) =>
    Database.use((tx) =>
      tx
        .select({
  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on Zingpop`,
          body: render(
            // @ts-ignore
            InviteEmail({
  ...
  export const joinInvitedWorkspaces = fn(z.void(), async () => {
    ...
    const email = account.properties.email
    if (!email) return
